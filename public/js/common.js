...
$.ajax({
    ...
    success: function (data) {
        ...
        for (var i = 0; i < goods.length; i++) {
            goods[i].good_price = goods[i].good_price.toString();
            goods[i].idgood = goods[i].idgood.toString();
        }
    }
});
        ...
        time_array.push(
            {
                ...
                countgood : data[j].countgood.toString(),
                good_price : data[j].good_price.toString(),
                idgood: data[j].idgood.toString()
            }
          );
...
var tmp = JSON.stringify(orders);
  $.ajax({
    type: 'POST',
    url: 'save_order',
    data: {'categories': tmp, _token: CSRF_TOKEN},
...
function delete_order(id)
{
  this.id = id;
  $.ajax({
  data : { idorder : id , _token: CSRF_TOKEN },
  url : 'delete_order',
  global : false,
  type : "POST",
  dataType : "html",
